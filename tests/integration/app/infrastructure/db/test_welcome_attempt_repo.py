...
Дублёрами это не проверить: у сессии сценария и сессии журнала разные
транзакции и соединения, а с дублёром откатывать нечего.
"""
...
async def test_record_survives_caller_rollback(engine, session):
    ...
    """
    journal = SqlWelcomeAttemptRepo(AutonomousSession(engine))
