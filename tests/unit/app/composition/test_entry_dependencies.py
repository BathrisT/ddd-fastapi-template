...
# Ниже этого числа проверка сломана, а не «всё хорошо»: пустой список
# параметров pytest молча превращает в SKIPPED, и обвал сборщика выглядел бы
# зелёным прогоном. Держится близко к фактическому.
...
def _requested_types(func: Any) -> list[Any]:
    ...
    Читается ИСХОДНИК: обёртки ВЫРЕЗАЮТ эти параметры из сигнатуры и не
    выставляют `__wrapped__`, так что по живому объекту список всегда пуст.
    """
...
def _walk(router: Any) -> tuple[list[APIRoute], list[Any]]:
    ...
    Проверки собираются отдельно: подключённых на роутере нет в `dependant`
    агрегатора, а `app.routes` разворачивается лениво и даёт ноль.
    """
