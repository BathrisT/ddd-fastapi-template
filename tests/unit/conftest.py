...
class FakeUserRepo:
    ...
    Наружу и внутрь уходит КОПИЯ: отдай дублёр тот же объект, и сценарий с
    `command.id` вместо `saved.id` был бы зелёным, а в проде ушёл бы с
    `user_id=0`. Поблажка одна: `created_at` хранится как передали.
    """
