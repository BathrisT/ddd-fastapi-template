...
Живёт до рестарта. Состояние арендатора здесь держать нельзя — иначе первый
пришедший определит объект для всех остальных, молча. Файл защищает от скоупа,
собранного дважды и по-разному: в `lifespan` и самодельно в модуле брокера.
...
class ProcessProvider(Provider):
    ...
    def session_factory(self, engine: AsyncEngine) -> async_sessionmaker[AsyncSession]:
        """`expire_on_commit=False`: с `True` обращение к атрибуту после
        коммита дёргает ленивую подгрузку, а в async это `MissingGreenlet`.
        `autoflush=False`: иначе висящая вставка уезжает внутри следующего
        запроса, и `IntegrityError` летит мимо сделавшего её репозитория.
        """
    ...
    async def http(self) -> AsyncIterator[httpx.AsyncClient]:
        ...
        Процессный ровно пока не знает про арендатора: `base_url` тут не
        задаётся, адрес и токен живут в привходовой обёртке.
        """
