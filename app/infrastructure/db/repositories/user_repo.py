...
class SqlUserRepo(UserRepo):
    ...
    async def save(self, user: User) -> User:
        ...
        try:
            ...
            # Перевод в доменный отказ здесь и только здесь: `IntegrityError`
            # потянул бы в сценарий имя констрейнта. Отката НЕ делаем — сессией
            # владеет контейнер, а свой `rollback()` отменил бы заодно всё, что
            # сценарий успел сделать до этого.
