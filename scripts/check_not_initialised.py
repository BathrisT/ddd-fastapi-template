...
`make init` — единственное обязательное действие после клонирования и
единственное, которое нечем напомнить: README можно не прочитать, а красную
проверку нет. Отличить шаблон от свежего клона по содержимому невозможно —
деревья байт в байт одинаковы, — поэтому признак «я и есть шаблон» живёт вне
git: локальный `.is-template`. Закоммиченный маркер приехал бы в клон и молчал
бы ровно там, где нужен голос.
