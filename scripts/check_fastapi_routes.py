...
def check_no_deferred_annotations(config: dict) -> list[str]:
    ...
    `from __future__ import annotations` делает `-> XResponse` СТРОКОЙ, а у
    обёрнутого хендлера `__globals__` уже свои. Отказ тихий: тесты зелёные, а
    маршрут отдаёт 500 на сериализации ОТВЕТА — так слегло двадцать разом.
    """
    roots = config.get("route_dirs", [])
    methods = set(config.get("http_methods", []))
    if not roots or not methods:
