...
def must(*args: str) -> str:
    ...
    `out()` код возврата выбрасывает, и для отчётов это нормально. Но упавший
    `git status` прочитался бы как «дерево чистое» и пустил слияние поверх
    незакоммиченного, а упавший `git log` — как «шаблон не ушёл вперёд».
    """
    result = git(*args)
...
class Template:
    ...
    def alerting(self, paths: list) -> list:
        """Входящие файлы, опасные САМИМ фактом появления, без конфликта.

        Миграция из шаблона сольётся чисто — она новая. Но цепочка ревизий у
        шаблона своя, и после слияния голов станет две: `alembic upgrade head`
        перестанет работать, а отчёт «конфликтов не будет» формально прав.
        """
...
class Sync:
    ...
    def dirty(self) -> bool:
        ...
        `--untracked-files=no` намеренно: теряется несохранённая работа, а не
        всякий лишний файл. Иначе отказ прилетал бы из-за `__pycache__`,
        который создаёт сам же запуск сторожа. Затирание untracked ловит git.
        """
