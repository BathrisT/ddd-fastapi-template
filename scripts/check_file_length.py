...
Правило «один файл — одна логическая группа» не проверялось, и файл дорос до
1244 строк, собрав меню, карточки, тесты, опросники и цели. Форма кода тут ни
при чём — класс с 88 методами был бы той же помойкой; объём ловится лимитом.

Порог — `[tool.code_layout].max_lines`; медиана файла в проекте 33 строки, так
что 300 это девять медиан. В ruff такого правила нет, а тащить pylint ради
одного дороже. Проверяется весь `app/`, включая роутеры: контракт фреймворка
